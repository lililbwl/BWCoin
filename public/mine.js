...
function Block(timestamp, transactions, previousHash = ''){
	...
	bObj.createHashData = function(){
		...
		return CryptoJS.SHA256(structStr).toString();
	}
	...
	blockchainObj.getBalanceOfAddress = function(address){
  	...
  	blockchainObj.isChainValid = function(){
    	for (let i = 1; i < blockchainObj.chain.length; i++){
			...
			currentStr = currentBlock.previousHash + currentBlock.timestamp + JSON.stringify(currentBlock.transactions)+currentBlock.nonce;
      		if (currentBlock.hashData !== CryptoJS.SHA256(currentStr).toString()) {
				// console.log("!");
				return false;
